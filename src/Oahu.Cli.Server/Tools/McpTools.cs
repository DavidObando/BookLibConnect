...
public sealed class McpTools
{
    ...
    [Description("List books in the local library cache. `filter` matches title/author/series substring; `limit` caps results.")]
    ...
    [Description("Add one or more ASINs to the queue. `quality` is High|Normal|Low. `title` is honoured only when adding a single ASIN.")]
    ...
    [Description("Submit a download for one or more ASINs. Returns immediately with the assigned jobIds; poll `jobs_status` to track progress.")]
    ...
    [Description("Run environment self-checks (write perms, profile store readable, library cache reachable, Audible API reachable, disk free).")]
    ...
    [Description("Set one config key. Allowed keys: DownloadDirectory, DefaultQuality, MaxParallelJobs, KeepEncryptedFiles.")]
